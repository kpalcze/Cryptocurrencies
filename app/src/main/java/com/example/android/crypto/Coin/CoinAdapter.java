...
public class CoinAdapter extends RecyclerView.Adapter<CoinAdapter.ViewHolder> {
    ...
    public CoinAdapter(List<Coin> data, SharedPreferences sharedPreferences) {
        this.data = data;
    ...
    public void onBindViewHolder(CoinAdapter.ViewHolder holder, int position) {
        ...
        if (sharedPreferences.contains(data.get(position).getId())) {
            holder.coinFavourite.setImageResource(R.drawable.ic_favourite_yello_24dp);
        } else {
            holder.coinFavourite.setImageResource(R.drawable.ic_favorite_white_24dp);
        }
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        @BindView(R.id.coin_favourite) ImageView coinFavourite;
        ...
        @OnClick(R.id.coin_favourite)
        void onClick(View view) {
            System.out.println("esssa" + getAdapterPosition());
            SharedPreferences.Editor editor = sharedPreferences.edit();
            String coinId = data.get(getAdapterPosition()).getId();
            if (sharedPreferences.contains(coinId)) {
                coinFavourite.setImageResource(R.drawable.ic_favorite_white_24dp);
                editor.remove(coinId);
            } else {
                coinFavourite.setImageResource(R.drawable.ic_favourite_yello_24dp);
